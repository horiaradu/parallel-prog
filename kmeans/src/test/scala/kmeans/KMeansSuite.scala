...
class KMeansSuite extends FunSuite {
  ...
  test("'kMeans' should work for 'points' == GenSeq((0, 0, 1), (0,0, -1), (0,1,0), (0,10,0)) and 'oldMeans' == GenSeq((0, -1, 0), (0, 2, 0)) and 'eta' == 12.25") {
    ...
    //    assert(classified === GenMap(new Point(0, -1, 0) -> GenSeq(new Point(0, 0, 1), new Point(0, 0, -1)), new Point(0, 2, 0) -> GenSeq(new Point(0, 1, 0), new Point(0, 10, 0))))
    val updated: GenSeq[Point] = update(classified, means)
    //    assert(updated === GenSeq(new Point(0, 0, 0), new Point(0, 5.5, 0)))
    assert(converged(eta)(means, updated) === true)
    ...
    assert(actual === Vector((0.0, 0.0, 0.0), (0.0, 5.5, 0.0)))
  }
