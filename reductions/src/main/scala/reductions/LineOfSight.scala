...
object LineOfSight {
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    for (i <- from until until) {
      val currentAngle = if (i != 0) input(i) / i else 0.0f
      max = Math.max(max, currentAngle)
    }
    max
  }
    ...
    tree match {
      ...
      case Node(leftTree, rightTree) =>
        parallel(
          ...
          downsweep(input, output, Math.max(startingAngle, leftTree.maxPrevious), rightTree)
        )
    }
  }
